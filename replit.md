...
**Classificacao topologica de paredes externas vs internas** (`pdfVectorExtractor.classifyWalls`): em vez de margem da bbox (15%), usa raio-casting perpendicular a partir do meio de cada parede. Se ao menos UM lado alcanca a borda sem cruzar outra parede → externa (face para o exterior). Se AMBOS os lados cruzam paredes → interna (cômodo dos dois lados). Funciona em casas em L, com garagem, varandas, e contornos irregulares. O prompt do Gemini (Etapa 3 + verificacao) reforca a regra topologica absoluta: "interna NUNCA fora do poligono externo" e instrui uso prioritario de cotas para validar comprimentos.
